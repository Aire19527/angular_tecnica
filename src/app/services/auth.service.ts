...
export class AuthService {
  ...
  login(user: UserModel) {
    return this.http.post(`${this.url}Login/Login`, user).pipe(
      map((resp) => {
  ...
  searchCompany(idEmpresa: number) {
    ...
    return this.http
      .get(
        `${
          this.url
        }Company/GetCompany?identificacionNumber=${idEmpresa.toString()}`,
        { headers: headers }
      )
      .pipe(
        map((resp) => {
  ...
  getAllTypeIdentification() {
    ...
    return this.http
      .get(`${this.url}/Transversal/GetAllTypeIdentification`, {
        headers: headers,
      })
      .pipe(
        map((resp) => {
  ...
  updateCompany(company: CompanyModel) {
    ...
    return this.http
      .post(`${this.url}Company/UpdateCompany`, company, { headers: headers })
      .pipe(
        map((resp) => {
          console.log(resp);
          return resp;
